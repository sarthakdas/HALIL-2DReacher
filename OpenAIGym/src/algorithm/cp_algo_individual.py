...
def sub_sample_demonstrations(deomonstrations: List[List[int]], num_samples: int, demonstration_indexes: List) -> List[List[int]]:
    '''
...
def query_llm(starting_position: List[int], goal_position: List[int], arm_lengths: List):
    '''
    ...
    help_needed, outputdictionary = llm_client.process_ensemble_training("src/tmp/prompt.txt", n = 5, temperature=0.5)

 
    # conver the token trajectory to waypoints
    for i in range(len(outputdictionary)):
        waypoints: list[list[int]] = extract_waypoints(outputdictionary[i]["action_token_trajectory"])["waypoints"]
        for j in range(len(waypoints)):
            waypoints[j] = [x / 100 for x in waypoints[j]]
        outputdictionary[i]["waypoints"] = waypoints

    
    # find the best prediction with the highest logprob
    best_prediction = outputdictionary[0]
    for i in range(1, len(outputdictionary)):
        if outputdictionary[i]["logprob"] > best_prediction["logprob"]:
            best_prediction = outputdictionary[i]
...
def cp_algo(env, current_state) -> Tuple[float, float]:
    if not hasattr(env, 'llm'):
        waypoints, env.ensomble_dictionary = query_llm(
