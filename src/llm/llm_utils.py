...
class OpenAIClient:
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    model='ft:gpt-4o-mini-2024-07-18:personal:2d-reacher-1500:9viuBzDt',  # Consider updating the model if using an updated API: gpt-3.5-turbo, gpt-4o, gpt-4o-mini, ft:gpt-4o-mini-2024-07-18:personal:2d-reacher-1500:9viuBzDt
    ...
    # def process_cp(self, demonstation_file_path,  num_of_ensoemble_paths=5):
        '''
        ...
        confidence_level = 0.29 # for 5 options
        qhat = confidence_level
    ...
    def process_test(self, demonstation_file_path):
    ...
    def process_ensemble_training(self, demonstation_file_path, n=1 ,temperature=0):
        ...
        response = None
        while response is None:
            response, text = self.lm(self.system_prompt, logit_bias=None, n=n, temperature=temperature)
            if response is None:
                time.sleep(60)
        ...
        generated_pathways = []
        for i in range(n):
            generated_pathways.append(response.choices[i].message.content)

        random.shuffle(generated_pathways)
        return generated_pathways

    def get_softmax_of_pathways (self, demonstation_file_path, generated_pathways, input: str):
        '''
        Get the softmax of the generated pathways

        demonstation_file_path: The path to the file containing the demonstration
        generated_pathways: The generated pathways
        return: The softmax of the generated pathways
        '''

        options = ['A', 'B', 'C', 'D', 'E', 'F']
        # make cure the options is equal to the number of generated pathways
        options = options[:len(generated_pathways)]
        # convert to string with comma separated
        options = ' '.join(options)
        print("Options", options)
        print("Option type", type(options))

        # shuffle the generated pathways to avoid bias
        self.system_prompt = self.load_prompt(demonstation_file_path)
        self.system_prompt += "Based on the demonstrations above, pick the correct option below and respond with one character: " + str(options) + ". DO NOT ADD ANYTHING ELSE\n"
        # self.system_prompt += "Give the best answer based on the options below \n"
        self.system_prompt += "Input:" + input + "\n"
        # self.update_system_prompt(user_prompt, context_description)



        with open('_extracted_options_raw.txt', 'w') as f:
            for i in range(len(generated_pathways)):
                #  in the form of A), B), C), D), E)
                f.write(f'{chr(65+i)}) {generated_pathways[i]}\n')
                self.system_prompt += f'\n{chr(65+i)}) {generated_pathways[i]}'


        with open('_prompt_mc_full.txt', 'w') as f:
            f.write(self.system_prompt)

        response_processed = False
        while response_processed is False:
            try:
                logit_bias = {
                355: 100.0,  # A (with space at front)
                418: 100.0,  # B (with space at front)
                363: 100.0,  # C (with space at front)
                415: 100.0,  # D (with space at front)
                457: 100.0,  # E (with space at front)
                454: 100.0,  # F (with space at front)
                }
                # reduce the dictionary to the number of generated pathways
                logit_bias_updated = dict(list(logit_bias.items())[:len(generated_pathways)])
                print("Length of generated pathways", len(generated_pathways))
                print("logitbias", logit_bias_updated)

                # save the ssytem prompt to a file
                with open('_prompt_before_softmax.txt', 'w') as f:
                    f.write(self.system_prompt)


                response, text = self.lm(self.system_prompt, max_tokens=1, logprobs=True, top_logprobs=len(generated_pathways), response_format={"type": "text"}, logit_bias=logit_bias_updated)
                if response is None:
                    time.sleep(60)

                with open('_multiple_choice.json', 'w') as f:
                    json.dump(str(response), f)


                top_logprobs_full = response.choices[0].logprobs.content[0].top_logprobs
                top_tokens = [token.token for token in top_logprobs_full]
                top_logprobs = [token.logprob for token in top_logprobs_full]
                ...
                print('\n====== Raw log probabilities for each option ======')
                log_probs_dict = {}
                for token, logprob in zip(top_tokens, mc_smx_all):
                    print('Option:', token, '\t', 'log prob:', logprob)
                    log_probs_dict[token] = {
                        "logprob": logprob,
                        "trajectory": generated_pathways[ord(token) - 65]
                    }
    ...
    def create_sub_set(self, log_probs_dict, confidence_level=0.29):
        # Construct prediction_set and pathways in a single iteration
        prediction_set = []
        pathways = []
        
        for token, data in log_probs_dict.items():
            logprob = data["logprob"]
            trajectory = data["trajectory"]
            is_pred = logprob >= confidence_level
            
            if is_pred:
                prediction_set.append(ord(token) - 65)
            
            pathways.append({
                "action_token_trajectory": trajectory,
                "logprob": logprob,
                "prediction_set": is_pred
            })
        
        # Print softmax scores and prediction set
        softmax_scores = [data["logprob"] for data in log_probs_dict.values()]
        print('Softmax scores:', softmax_scores)
        print('Prediction set:', prediction_set)
        
        # Determine if help is needed
        help_needed = len(prediction_set) != 1
        if help_needed:
            print('Help needed!')
        ...
        return help_needed, output_dict
...
if __name__ == "__main__":
    ...
    scene_objects = "{'object': [0, -4, 20], 'object_orientation': [4, -261, 1, 297]}"
    ...
    help, output = client.process_ensemble_training("_demo_data.txt",instruction, scene_objects, context_description,n=5,temperature=0.5)  
